...
def parse_feed(driver, sector, url):
    time.sleep(1)
    xml = fetch_xml(driver, url)
    #print(xml)
...
def sector_scrape(sector,url):
    count = 0
    driver =   get_driver()
    for rec in parse_feed(driver, sector, url):

        pprint(rec, indent=3, width=150)
        count += 1
        article =  scrape_bs_article(rec['url'])

        article_id = get_deterministic_uuid_from_url(rec['url'])
        headline = article['title']
        body = article['body']
        embeded_points = embed_article(article_id, headline, body,rec['url'])
        insert_points(embeded_points)
...
def main():
    ...
    for sector,url in FEEDS.items():
        sector_scrape(sector, url)
